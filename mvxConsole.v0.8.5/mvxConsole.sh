...
# link to mvx source files, which links remaining required files

# SCRIPTPATH is an important variable in original setup, but was illogical
# Rewriting and resetting and relying on mvx*Folder for needed paths
# Simple logic: this mvxConsole.sh, like script.sh is ALWAYS in /elrond-go-scripts
...
# This enables: mvxHomeFolder, mvxScriptFolder, mvxConfigFolder

# There should only be one elrond-go-scripts folder, use that as our base.
# This will work from any starting folder.
currentFolder=$(find ~ -name "elrond-go-scripts" -type d)

source $currentFolder"/config/mvx-Utility.cfg"
source $currentFolder"/config/mvx-Init.cfg"

# mvxScriptFolder gets set in mvx-Init.cfg
SCRIPTPATH=$mvxScriptFolder
...
#        read -p "This version of mvxConsole ignores any passed parameters." ignore
        tapToContinue "This version of mvxConsole ignores any passed parameters."
fi
